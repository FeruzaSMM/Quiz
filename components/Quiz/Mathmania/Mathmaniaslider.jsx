...
import Mathmaniaintro from "./Mathmaniaintro";

SwiperCore.use([Navigation]);

const Mathmaniaslider = (data) => {
                                        ...
                                        <Swiper {...swiperOption}>
                                            {data.data.length > 0 &&
                                                data.data.map((subcat, key) => {
                                                    return (
                                                        <SwiperSlide key={key}>
                                                            <Mathmaniaintro data={subcat} url={data.url} subcategoryid={subcat.id} active={data.selected && data.selected.id === subcat.id ? "active-one" : "unactive-one"} />
                                                        </SwiperSlide>
                                                    );
                                                })}
                                        </Swiper>
...
export default withTranslation()(Mathmaniaslider);
